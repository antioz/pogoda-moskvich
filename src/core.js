...
// Курсив в legacy Markdown — это _…_ (а *…* было бы жирным).
function guideText(weatherLine) {
  return [
    "блиииин, это погода для москвичей.",
    "каждое утро ною тебе какая на улице дрянь( в 07:00, если проснусь.",
    "",
    "хочешь нытьё себе в чат:",
    "• в группу — добавь меня обычным участником;",
    "• в канал — добавь админом с правом публикации.",
    "",
    `сегодня вот: _${weatherLine}_`,
  ].join("\n");
}
...
function groupGreetText(weatherLine) {
  return [
    "блиииин, это погода для москвичей.",
    "буду ныть сюда каждое утро в 07:00, если проснусь(",
    "надоест — просто удали меня из чата.",
    "",
    `сегодня вот: _${weatherLine}_`,
  ].join("\n");
}
